...
public:
    ...
    Layer(size_t num_neurons,size_t num_inputs,Callback activation_fn,Callback fnd,ErrorFun errfn) : weights(num_inputs,num_neurons),
	   bias(1,num_neurons),aL(1,num_neurons)
	{
    ...
    const Matrix& forward(const Matrix& inputs)
	{
		...
		aL.addrow(bias);
		//if(fn)
		//	fn(res,res); //activate output
		return aL;
	}
    // deltas at output layer
    const Matrix& deltas_output(const Matrix& y)
	{
		static Matrix ld(y.num_rows(),y.num_cols());
		static Matrix aL_dup(aL.rows,aL.cols);
        this->aL_dup = &aL_dup; 
		if(!fnd)
			errfnd(ld,aL,y);
		else 
	...
	{
        static Matrix dout_dup(dout.rows,dout.cols);
		static Matrix transpose(dw.rows,dw.cols);
		
		dw.transpose(transpose);
		dout.matmul(transpose,dout_dup);
		if(fnd)
		{
			fnd(aL,*aL_dup);
			dout_dup.mul(*aL_dup);
    ...
    void backpropagate(const Matrix& a,const Matrix& del,size_t idx,double lr = 1)
	{
