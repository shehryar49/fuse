...
void XOR_demo() {
    ...
    model.add_layer(128, 2, sigmoid_activation);
    model.add_layer(64, 128, sigmoid_activation);
    model.add_layer(1, 64, sigmoid_activation);

    size_t epochs = 1000;
    model.fit(inputs, y, epochs, 0.1);
}
void IRIS_demo() {
    double inputs_arr[150][4] = {
    ...
    Model m(mse_loss);
    m.add_layer(2, 4, sigmoid_activation);
    m.add_layer(3, 2, sigmoid_activation);
